...
export function SenderManagement() {
  ...
  async function load() {
    ...
    try {
      ...
      setRole(payload?.meta?.role ?? 'operator');
      setError(null);
    } catch (loadError) {
  ...
  async function verifyDomain(sender: EmailSender, check = false) {
    setSaving(sender.id); setError(null); setNotice(null);
    try {
      const suffix = check ? '/check' : '';
      const response = await fetch(`/api/senders/${encodeURIComponent(sender.id)}/verification${suffix}`, { method: 'POST' });
      const payload = await response.json().catch(() => null);
      if (!response.ok) throw new Error(payload?.error ?? 'Domain verification check failed');
      const record = payload?.data?.record;
      setNotice(check ? 'DNS check completed.' : `Add TXT ${record?.name ?? ''} = ${record?.value ?? ''}, then use Check DNS.`);
      await load();
    } catch (verifyError) {
      setError(verifyError instanceof Error ? verifyError.message : 'Domain verification check failed');
    } finally { setSaving(null); }
  }

  return <section className="panel tabPaneWide">
    <div className="panelHeader"><div><h2 className="panelTitle">Sender management</h2><p className="panelNote">Current scope: workspace <code>default</code> · team <code>outreach</code>. Only the configured internal domain is accepted.</p></div><span className="pill pillStrong">{role} access</span></div>
    <div className="panelBody stack">
      ...
      {!loading && senders.length > 0 && <div className="senderManagementList">{senders.map((sender) => <article className="senderManagementItem" key={sender.id}>
        ...
        <div className="rowWrap"><div className="senderHint">Domain: {sender.domainVerified ? 'verified' : 'not verified'} · DNS: {sender.domainVerification?.status ?? 'unverified'} · Sender: {sender.senderVerified ? 'verified' : 'not verified'} · Mailbox: {sender.mailboxAccount?.mailboxEmail ?? 'not linked'} ({sender.mailboxAccount?.verificationStatus ?? 'unverified'})</div><button className="button buttonSmall" type="button" disabled={saving === sender.id} onClick={() => void verifyDomain(sender)}>Request DNS TXT</button><button className="button buttonSmall" type="button" disabled={saving === sender.id} onClick={() => void verifyDomain(sender, true)}>Check DNS</button></div>
      </article>)}</div>}
    </div>
  </section>;
}
