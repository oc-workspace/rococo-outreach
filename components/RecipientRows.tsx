        ...
        <div className="recipientList">
          {rows.map((row) => (
            <div className="recipientRow" key={row.id}>
              <div className="field"><label htmlFor={`${row.id}-contact`}>Contact</label><select id={`${row.id}-contact`} className="select" value={row.contactId} onChange={(event) => selectContact(row.id, event.target.value)}><option value="">Manual email</option>{contacts.map((contact) => <option key={contact.id} value={contact.id}>{contact.email}</option>)}</select><label htmlFor={`${row.id}-email`}>Target email</label><input id={`${row.id}-email`} className="input" value={row.email} onChange={(event) => onUpdateRow(row.id, { email: event.target.value, contactId: '' })} /></div>
